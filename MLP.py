...
class EconometricAnalyzer:
    ...
    def endogeneity_test(self, instrument_idx=0, endogenous_idx=1):
        ...
        try:
            ...
            # 第二阶段回归
            X_new = np.column_stack([self.X[:, :endogenous_idx], 
                                    X_hat, 
                                    self.X[:, endogenous_idx+1:]])
    ...
    def mechanism_analysis(self, economic_vars=None):
        ...
        shap_values = explainer.shap_values(self.X[:50])[0]
        ...
        shap.summary_plot(shap_values, self.X[:50], feature_names=self.feature_names)
...
if __name__ == "__main__":
    ...
    history = model.fit(
        ...
        callbacks=[EarlyStopping(patience=15, monitor='val_auc', mode='max', restore_best_weights=True)],
        verbose=1
    )
